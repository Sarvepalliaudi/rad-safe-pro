  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      ...
      {(view === 'DASHBOARD' || view === 'SECTION') && (
        <header className="fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-b border-gray-100 h-16 z-30 px-4 md:px-8 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-3">
            ...
            >
              <div className="bg-rad-600 p-1.5 rounded-lg text-white group-hover:rotate-12 transition-transform shrink-0">
                <ShieldCheck size={20} />
              </div>
              <h1 className="text-sm md:text-lg font-black tracking-tighter uppercase text-slate-900 truncate">RAD SAFE PRO</h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-3">
             ...
             <button 
               ...
               className="p-2 md:p-2.5 bg-slate-50 text-slate-500 rounded-xl hover:bg-rad-50 hover:text-rad-600 transition-all shadow-sm"
               title="About Project"
             >
               <Award size={20} />
             </button>
      ...
      <main className={`flex-1 transition-all duration-300 ${(view === 'DASHBOARD' || view === 'SECTION') ? 'pt-20 md:pt-24 md:pl-64' : ''}`}>
        <div className="px-4 md:px-8 max-w-7xl mx-auto w-full pb-20 md:pb-8">
          {renderContent()}
        </div>
      </main>
      ...
      {(view === 'DASHBOARD' || view === 'SECTION') && (
        <>
          <button 
            ...
            className="fixed bottom-20 md:bottom-6 right-4 md:right-6 p-4 md:p-5 bg-slate-900 text-white rounded-2xl md:rounded-[2rem] shadow-2xl hover:scale-110 active:scale-90 transition-all z-40 group border-2 md:border-4 border-white"
          >
            <div className="relative">
              <Bot size={24} className="md:w-7 md:h-7" />
              <div className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-rad-400 rounded-full border-2 border-slate-900"></div>
            </div>
          </button>

          {isRadOpen && (
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end md:items-center justify-center md:justify-end p-0 md:p-4 animate-fade-in">
              <div className="bg-white w-full max-w-md h-[90vh] md:h-[600px] rounded-t-[2.5rem] md:rounded-[3rem] shadow-2xl flex flex-col overflow-hidden border border-slate-100">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <p className="text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] opacity-60">Neural Engine Active</p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 bg-slate-50">
                  {radChat.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center text-center px-6 opacity-30">
                       ...
                       <p className="text-[10px] font-bold">How can I help you today?</p>
                    </div>
                  )}
                  {radChat.map(msg => (
                    <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div className={`p-3 md:p-4 rounded-2xl text-[11px] md:text-xs font-bold leading-relaxed shadow-sm max-w-[85%] ${msg.role === 'user' ? 'bg-slate-900 text-white rounded-tr-none' : 'bg-white text-slate-700 rounded-tl-none border border-slate-100'}`}>
                ...
                <div className="p-4 md:p-6 bg-white border-t border-slate-100">
                  <div className="relative">
                    <input 
                      ...
                      className="w-full p-4 pr-14 bg-slate-100 border-none rounded-2xl text-[11px] md:text-xs font-bold focus:bg-white focus:ring-2 focus:ring-rad-500 outline-none transition-all" 
                    />
                    <button 
      ...
      {/* MOBILE TAB BAR */}
      {(view === 'DASHBOARD' || view === 'SECTION') && (
        <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-gray-100 h-16 md:hidden z-30 flex items-center justify-around px-2 shadow-[0_-5px_20px_rgba(0,0,0,0.05)]">
           <button onClick={() => setView('DASHBOARD')} className={`flex flex-col items-center gap-1 p-2 transition-all ${view === 'DASHBOARD' ? 'text-rad-600' : 'text-slate-400'}`}>
              <ShieldCheck size={20} />
              <span className="text-[8px] font-black uppercase tracking-tighter">Home</span>
           </button>
           <button onClick={() => navigateTo('quiz')} className={`flex flex-col items-center gap-1 p-2 transition-all ${activeTab === 'quiz' ? 'text-orange-600' : 'text-slate-400'}`}>
              <HelpCircle size={20} />
              <span className="text-[8px] font-black uppercase tracking-tighter">Quiz</span>
           </button>
           <button onClick={() => navigateTo('calculator')} className={`flex flex-col items-center gap-1 p-2 transition-all ${activeTab === 'calculator' ? 'text-emerald-600' : 'text-slate-400'}`}>
              <Calculator size={20} />
              <span className="text-[8px] font-black uppercase tracking-tighter">Dose</span>
           </button>
